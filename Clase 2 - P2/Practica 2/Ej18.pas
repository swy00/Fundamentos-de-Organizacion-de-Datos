...
procedure RellenarCampos(var reg: reg_mae; regnac: reg_nac; regfal: reg_fal);
	begin
		...
		//Si la partida de nacimiento es menor que la de fallecimiento
		if (regnac.nroPartida < regfal.nroPartida) then begin
			...
			reg.fallecio:= false;
		//Si la partida de fallecimiento es menor que la de nacimiento
		end else if (regnac.nroPartida > regfal.nroPartida) then begin
		...
		//En el caso de que sean iguales, rellenar el registro con ambos registros
		end else begin 
			reg.nombre:= regnac.nombre;
			reg.apellido:= regnac.apellido;
			reg.direccion:= regnac.direccion;
